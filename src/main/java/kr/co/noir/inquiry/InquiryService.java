...
public class InquiryService {
	...
	public int totalCnt(BoardRangeDTO rDTO) {
		int totalCnt=0;
		try {
			totalCnt=iDAO.selectBoardTotalCnt(rDTO);
	 ...
	 * @return
	 */
	public List<InquiryDomain> searchBoardList(BoardRangeDTO rDTO){
		List<InquiryDomain> list=null; 
		try {
			list=iDAO.selectRangeBoard(rDTO);
		} catch (SQLException e) {
			e.printStackTrace();
		}//end catch
	 ...
	 * @return
	 */
	public InquiryDomain searchOneBoard(int inquiryNum) {
		InquiryDomain iDomain = null;
		try {
			iDomain= iDAO.selectBoardDetail(inquiryNum);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return iDomain;
	...
	/**
	 * 문의글 작성 (DB 저장)
	 * @param iDTO - 사용자가 입력한 제목, 내용, 회원번호가 담긴 객체
	 */
	public void addInquiry(InquiryDTO iDTO) {
	    try {
	        iDAO.insertInquiryByMember(iDTO);
	    } catch (SQLException e) {
	        e.printStackTrace();
	    }
	}
